...
// Static files
// Host welcome page
app.get('/welcome', cacheBuster.middleware, (request, response) => {
    return response.sendFile('welcome.html', { root: path.join(serverDirectory, 'public') });
});

// Host register page
    app.get('/register', cacheBuster.middleware, (request, response) => {
        return response.sendFile('register.html', { root: path.join(serverDirectory, 'public') });
    });

    app.get('/public-characters', cacheBuster.middleware, (request, response) => {
        return response.sendFile('public-characters.html', { root: path.join(serverDirectory, 'public') });
    });

    app.get('/forum', cacheBuster.middleware, (request, response) => {
        return response.sendFile('forum.html', { root: path.join(serverDirectory, 'public') });
    });

    app.get('/forum-test', cacheBuster.middleware, (request, response) => {
        return response.sendFile('forum-test.html', { root: path.join(serverDirectory, 'public') });
    });
...
app.get('/', cacheBuster.middleware, (request, response) => {
    if (shouldRedirectToLogin(request)) {
        ...
        const redirectUrl = query ? `/welcome?${query}` : '/welcome';
